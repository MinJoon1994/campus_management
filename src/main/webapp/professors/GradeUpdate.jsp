...
<script>
    ...
    const pageSize = 10;              // 한 페이지에 표시할 행 수
    ...
    // 성적 리스트에서 필터 조건(과목/학년/이름)에 맞는 학생들을 화면에 렌더링 + 페이징
	function searchGrades(page = 1) {
	    ...
	    pagedList.forEach(vo => {
	        ...
	        tr.querySelectorAll("input[type='number']").forEach(input => {
	            input.addEventListener("input", () => {
	                ...
	                tr.querySelector(".totalScore").innerText = total.toFixed(2); // 소수점 두 자리
	    ...
	    // 페이지네이션 버튼 생성
	    const maxPageButtons = 2; // 한 화면에 표시할 페이지 버튼의 최대 수
